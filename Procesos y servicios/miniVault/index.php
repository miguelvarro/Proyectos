...
function normalize_hash(string $hash): string {
  // Quita espacios y normaliza a minúsculas para comparar
  return strtolower(trim($hash));
}

function hash_equals_safe(string $a, string $b): bool {
  // hash_equals existe en PHP moderno; si no, fallback seguro
...
if ($export === 'csv') {
  ...
  // BOM opcional para Excel (mejor compatibilidad)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['modo'])) {
  ...
  try {
    switch ($modo) {
      ...
      // Hash
      case 'hash_md5':
        // permitimos vacío también (no pasa nada), pero normalmente habrá texto
        $resultado = md5($texto);
...
// refrescar historial tras posibles cambios
$history = $_SESSION['history'] ?? [];
if (!is_array($history)) $history = [];
