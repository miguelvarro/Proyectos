...
/**
 * admin.php — Administrador del servidor (versión estable)
 * Cambios clave:
 * - Arrancar/Parar monitor NO ejecuta powershell ni start-process desde PHP.
 * - En su lugar llama a scripts/monitor_runner.py (start/stop/status) y NO bloquea.
 * - Procesos se cargan bajo demanda (?procs=1) para evitar bloqueos.
 */

$CONFIG = require __DIR__ . '/../config/config.php';
$USERS = $CONFIG['users'] ?? ['miguel' => '1234'];

// Seguridad: por defecto NO permitimos matar procesos
...
$MONITOR_SCRIPT = $CONFIG['monitor_script']; // info
$RUNNER_SCRIPT  = $CONFIG['runner_script'];  // usado para start/stop/status
...
function get_uptime_string(): string {
  // En Windows evitamos WMI/Powershell (puede colgar). Best-effort:
  $os = os_family();
...
function launch_runner_background(string $runnerScript, string $cmd, string $logDir): array {
  ...
  // Comando: python "runner" start|stop|status
  $base = "python $runner $cmd";

  if ($os === 'Windows') {
    // start "" /B cmd /c "python runner start > log 2>&1"
    // Esto NO bloquea la petición PHP
