...
<script>
  // NOTA: el endpoint JSON requiere Basic Auth.
  // Para facilitar el dashboard sin pedir credenciales cada vez, aquí hacemos:
  // - Si el navegador no tiene auth, verás líneas vacías.
  // Solución típica: abrir una vez la URL del endpoint (con auth) y ya queda cacheado.
  ...
  function pointsToPolyline(points) {
    ...
    // map to viewBox: x 0..1000, y 0..200 (invert y)
  ...
  async function refreshAll() {
    ...
    for (const ep of endpoints) {
      ...
      try {
        ...
        // si no hay auth o no hay csv, dejamos la línea vacía
