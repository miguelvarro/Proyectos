...
def safe_filename(name: str) -> str:
    # Reemplaza caracteres inválidos en Windows: \ / : * ? " < > |  y controla espacios/puntos finales
...
def monitor_disk_usage(now_str: str):
    # En Windows, '/' a veces no es lo ideal. Intentamos primero '/', si falla, intentamos 'C:\\'
    try:
...
def monitor_processes(now_str: str, top_n: int = 10):
    ...
    for p in psutil.process_iter(attrs=["pid", "name", "username", "memory_info"]):
        try:
            ...
            # Cualquier proceso raro no debería tumbar el monitor
            continue

    # Ordena por memoria desc y recorta
    processes.sort(key=lambda r: r[-1], reverse=True)
    processes = processes[:top_n]

    # Importante: lo escribimos "en modo overwrite" para que sea snapshot actual (top N),
    # no una serie temporal gigantesca (si prefieres serie temporal, dímelo y lo cambiamos).
...
def monitor_apache_request_rate():
    ...
    if not os.path.isfile(ACCESS_LOG):
        # Si no existe el log, no revientes: no guardas nada y listo
        return
