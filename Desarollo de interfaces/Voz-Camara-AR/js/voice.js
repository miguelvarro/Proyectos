window.Voice = (() => {
  ...
  // Convierte "cero/uno/dos..." a índice
  ...
  // Aliases para que siga entrando por switch(operacion) pero reconozca variaciones
  ...
  function ejecutarOperacion(operacionRaw, parametro, fraseCompleta) {
    ...
    // Tu estilo total: switch por operación
    switch (operacion) {
  ...
  break;
      ...
      default:
        // Eco como tú: repito lo oído, pero guardo para "repetir"
  ...
  function init() {
    ...
    rec.onresult = function (e) {
      ...
      // Tu patrón: operación 1ª palabra, parámetro 2ª palabra
      const operacion = word(reconocido, 0);
      const parametro = word(reconocido, 1);

      // Si “voz” está desactivada, no ejecutamos nada (pero sí mostramos transcript)
...
rec.onerror = (ev) => {
  ...
  // Mensajes útiles (para ti y para el profe)
  ...
  if (code === "network") {
    // Lo evitamos si se intenta empezar demasiado rápido
    const now = Date.now();
    if (now - lastStartAt < 800) return;

    // Reintento suave (máximo 2)
    if (retryCount < 2) {
      retryCount++;
      setTimeout(() => {
        try {
...
rec.onend = () => {
  // reset de reintentos cuando termina correctamente
  retryCount = 0;
};
